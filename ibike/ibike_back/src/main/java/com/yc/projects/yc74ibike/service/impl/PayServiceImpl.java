...
public class PayServiceImpl implements PayService {
	...
	public void pay(PayModel payModel) {
		...
		// 2. 将数据保存到mongo的 payLog ( uuid,phoneNum,openId, 结账时间(年月日小时) 起(经纬),时间, 止(经纬)
		// ,时间, 花费)
		this.mongoTemplate.insert(payModel, "payLog");
		// 3. 修改单车的经纬度, 状态为1
		Query q = new Query(Criteria.where("id").is(payModel.getBid()));
		Update u = new Update().set("latitude", payModel.getEnd_latitude()).set("longitude",
				payModel.getEnd_longitude());
		Double[] loc = new Double[] { payModel.getEnd_latitude(), payModel.getEnd_longitude() };
